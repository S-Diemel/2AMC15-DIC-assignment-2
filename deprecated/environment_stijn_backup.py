...
def sample_points_in_rectangles(rectangles, number_of_items, radius):
    """sample package points in package area, and delivery points in areas around storage racks"""
    points = []
...
def sample_one_point_outside(rectangles, radius, bounding_rect):
    """Sample agents starting position"""
...
class WarehouseEnv(gym.Env):
    ...
    def __init__(
        ...
        self.width = width  # warehouse width
        self.height = height  # warehouse height
        self.agent_radius = agent_radius  # radius of the agent
        self.step_size = step_size  # step size of the agent
        self.speed = 0  # right now we work with steps of certain size not with speed
        self.orientation = 0
        self.agent_angle = 90  # The number of degrees the agent makes. Right now it can only turn in place and then move forward. No diagonal movement with 90 degrees. 
        # Obstacles: list of (x_min, y_min, x_max, y_max)
        self.racks = rack_obstacles or [  # Storages racks rectangles
            (4.0, 2.0, 4.5, 6.0),  # example rack
            (6.0, 4.0, 6.5, 10.0),
        ]
        self.box_obs = box_obstacles or [  # Box obstacles rectangles --> used for any other obstacles like humans, boxes, etc.
            (1.0, 5.0, 2.0, 6.0),
            (7.0, 1.0, 8.0, 2.0),
        ]
        self.charger = (4,9,6,10)
        self.forbidden_zone = (0,8,2,10)  # develop multiple zones
