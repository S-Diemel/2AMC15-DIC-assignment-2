...
def main(name: str, no_gui: bool, episodes: int, iters: int, random_seed: int, epsilon: float,
    ...
    agent = PPOAgent(state_size=14, action_size=6, seed=random_seed, num_envs=num_envs)
    ...
    for episode in range(episodes // num_envs):
        ...
        print(f"Episode batch {episode + 1}/{episodes // num_envs}")

        if not no_gui and (episode+1) % 100 == 0 and episode != 0:
            print(f"Time since last gui: {time.time() - start_time}")
            start_time = time.time()
        ...
        for _ in range(iters):
            ...
            # Break if all environments are done, truncated or terminated
