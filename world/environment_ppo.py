...
class Environment(gym.Env):
    ...
    def __init__(
            ...
            sigma=0,
            difficulty=None,
            # Difficulty level represented by None (random no particular difficulty level), 0 (easy), 1 (medium), 2 (hard)
            extra_obstacles=None,
            # List of additional obstacles, if any --> useful for experimenting with humans or boxes in random places
    ):
        super().__init__()
        ...
        assert len(
            self.item_spawn) == 1, "The environment allows only a single item spawn, due to some inherent design choices and scope limitation."
        ...
        self.racks = [  # Blue Areas: storage racks
            ...
            (self.width - half_width_of_rack, half_width_of_rack, self.width, self.height - half_width_of_rack),
            # right
        ...
        self.forbidden_zones = [(14, self.height - 12 * half_width_of_rack, 17.5,
                                 self.height - 8 * half_width_of_rack)]  # Red Area: forbidden zones, where the agent can but should not go
        ...
        self.item_spawn_center = ((self.item_spawn[0][0] + self.item_spawn[0][2]) / 2,
                                  (self.item_spawn[0][1] + self.item_spawn[0][3]) / 2)
    ...
    def reset(self, no_gui=True, seed=None, agent_start_pos=False, difficulty=None, extra_obstacles=None,
              number_of_items=None, battery_drain_per_step=None, options=None):
        """
        ...
        self.difficulty_region = set_difficulty_of_env(
            self.item_spawn, self.width, self.height,
            self.difficulty)  # For curriculum learning set the difficulty of the environment
        self.item_starts = sample_points_in_rectangles(
            self.item_spawn, self.number_of_items, self.item_radius,
            self.all_obstacles)  # spawn/initialize packages/items
        self.delivery_points = sample_points_in_rectangles(
            self.delivery_zones, self.number_of_items, self.delivery_radius, self.all_obstacles,
            self.difficulty_region)  # choose delivery spots
    ...
    def step(self, action):
        ...
        """
        env_stochasticity = np.random.choice([True, False], p=[self.sigma,
                                                               1 - self.sigma])  # determine whether to do apply environment stochasticity (slippery)
        ...
        info = {
            "env_stochasticity": env_stochasticity}  # we need to supply as step "info" parameter, we add information regarding stochasticity of the step
        ...
        self.orientation, new_pos = calc_new_position(action, self.speed, self.orientation, self.agent_angle,
                                                      self.agent_pos, self.step_size)
        self.agent_pos, collided = calc_collision(old_pos, new_pos, self.agent_radius, self.width, self.height,
                                                  self.all_obstacles)
        ...
        if env_stochasticity and not collided:  # if not collided, new_pos and correct_new_pos are the same --> intuition slippery-ness only has effect when no collisions happen
            ...
            self.orientation, new_pos = calc_new_position(action, self.speed, self.orientation, self.agent_angle,
                                                          self.agent_pos, self.step_size)
            self.agent_pos, collided = calc_collision(old_pos_extra, new_pos, self.agent_radius, self.width,
                                                      self.height, self.all_obstacles)
    ...
    def _compute_features(self):
        ...
        steps_fw = compute_distance_to_wall(self.orientation, self.max_range,
                                            self.agent_radius, self.agent_pos, self.width, self.height,
                                            self.all_obstacles)
        ...
        steps_fw_left = compute_distance_to_wall((self.orientation - self.agent_angle) % 360,
                                                 self.max_range, self.agent_radius, self.agent_pos, self.width,
                                                 self.height, self.all_obstacles)  # Left 45
        steps_fw_right = compute_distance_to_wall((self.orientation + self.agent_angle) % 360,
                                                  self.max_range, self.agent_radius, self.agent_pos, self.width,
                                                  self.height, self.all_obstacles)  # Right 45
        ...
        #     self.max_range, self.agent_radius, self.agent_pos, self.item_starts, self.item_radius, self.delivered, self.carrying)
        vision_triangle_sensor = calc_dist_to_item_in_triangle(self.agent_pos, self.max_range, self.agent_radius,
                                                               self.item_starts, self.delivered, self.carrying,
                                                               self.vision_triangle, self.all_obstacles)
        # Binary indicator whether agent is carrying an item
        can_interact = calc_can_interact(self.agent_pos, self.agent_radius, self.items, self.item_radius,
                                         self.delivery_points, self.delivery_radius, self.delivered, self.carrying,
                                         self.charger)
        if self.carrying >= 0:
            carrying = 1
        else:
        ...
        dist_target_x, dist_target_y = compute_dist_to_target(self.agent_pos, self.battery,
                                                              self.battery_value_reward_charging,
                                                              self.charger_center, self.carrying, self.delivery_points,
                                                              self.item_spawn_center)
